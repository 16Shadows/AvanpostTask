# Примечании реализация
При реализации задания были сделаны следующие предположения, которые по-хорошему необходимо бы было уточнить/проанализировать:
1. Структура ID прав, возвращаемых коннектором, имеет значение и должна быть неизменной.
2. Сервис, с которым взаимодействует коннектор, возвращает 401 Not Authorized, если токен авторизации просрочен.
3. Сервис возвращает expires_in в милисекундах.
4. Сервис несёт ответственность за отклонение некорректных операций (получение/изменение заблокированного пользователя).
